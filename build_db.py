...
DEPARTMENTS = [("تقنية المعلومات",24,800),("الموارد البشرية",8,800),
               ("المالية",11,800),("التسويق",15,800),("العمليات",32,800)]
def build(path=DB_PATH):
    p = Path(path)
    if p.exists(): p.unlink()
    conn = sqlite3.connect(p); cur = conn.cursor()
    cur.execute("""CREATE TABLE employees (employee_id TEXT PRIMARY KEY, name TEXT NOT NULL,
        department TEXT NOT NULL, job_title TEXT NOT NULL, annual_leave INTEGER NOT NULL,
        leave_used INTEGER NOT NULL)""")
    cur.execute("""CREATE TABLE departments (name TEXT PRIMARY KEY, headcount INTEGER NOT NULL,
